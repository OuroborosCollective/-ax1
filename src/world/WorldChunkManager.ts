...
export class WorldChunkManager {
  ...
  constructor(scene: THREE.Scene) {
    ...
    this.loadSavedChunksFromStorage();
  }

  /**
   * Bootstraps the central chunk so coordinate (0,0) is recognized in the persistent world map.
   */
  ...
  /**
   * Loads saved chunks from server backend & localStorage fallback.
   */
  public async loadSavedChunksFromStorage(): Promise<void> {
    try {
      const res = await fetch('/api/world/chunks');
      if (res.ok) {
        const data = await res.json();
        if (Array.isArray(data.chunks)) {
          for (const chunk of data.chunks) {
            if (!this.chunks.has(chunk.chunkKey)) {
              this.chunks.set(chunk.chunkKey, chunk);
              if (chunk.chunkKey !== '0,0') {
                this.renderChunkMeshes(chunk);
              }
            }
          }
          console.info(`[WorldChunkManager] Loaded ${data.chunks.length} persistent chunks from server database.`);
          return;
        }
      }
    } catch (err) {
      console.warn('[WorldChunkManager] Server chunk fetch failed, falling back to local storage cache:', err);
    }

    // Local Storage Fallback
    try {
      const local = localStorage.getItem('aurion_saved_world_chunks');
      if (local) {
      ...
      console.warn('[WorldChunkManager] Local chunk read error:', e);
    }
  }

  /**
   * Persists a newly constructed chunk to backend and localStorage.
   */
  private async persistChunk(chunk: WorldChunkData): Promise<void> {
    // 1. Local storage cache
    try {
      const allChunks = Array.from(this.chunks.values());
      localStorage.setItem('aurion_saved_world_chunks', JSON.stringify(allChunks));
    } catch (e) {
      console.warn('[WorldChunkManager] Could not save chunk to localStorage', e);
    }

    // 2. Server database persistence
    try {
      await fetch('/api/world/chunks', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ chunk }),
      });
    } catch (e) {
      console.warn('[WorldChunkManager] Failed to post chunk to server API', e);
    }
  }
   ...
   * generates and saves the new territory into permanent world logic!
   */
  ...
  /**
   * Builds a new chunk, creates its geometry with solid colliders,
   * and saves it to permanent world logic.
   */
  public buildAndSaveChunk(cx: number, cz: number): WorldChunkData {
    ...
    // Persist to server and client storage
    this.persistChunk(chunkData);
