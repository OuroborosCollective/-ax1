...
} from "./ax1LivingWorldProtocol";
import type { NpcGoal, NpcNeedEvent } from "./npcNeeds";
import type { PolityGovernmentType, WorldSignal } from "./worldPolityRules";

export type { HubId, MarketState, NpcEconomyState, LivingWorldResolution, LivingWorldSocialAction };

export type NpcLifeOpportunity = Readonly<{
  id: string;
  kind: "safe_hub" | "resource" | "social" | "reputation" | "market" | "influence";
  regionId: string;
  targetId?: string;
  benefitBps: number;
  riskBps: number;
...
export type NpcEconomySnapshot = Readonly<{
  npcId: string;
  decision?: {
    goal?: NpcGoal;
  };
...
export type MerchantDecisionRequests = Readonly<{
  resolution: LivingWorldResolution;
...
export function npcName(regionId: HubId): string {
  return regionId === "emberfall"
    ? "Torin"
    : regionId === "windhollow"
    ? "Elowen"
    : regionId === "cinder_vault"
    ? "Kael"
    : "Valen";
}
...
export function defaultNpc(regionId: HubId): NpcEconomyState {
...
export function confirmedNpcEconomy(homeRegionId: HubId, snapshot: NpcEconomySnapshot | null): NpcEconomyState {
  if (!snapshot || !snapshot.lifeState?.economy) return defaultNpc(homeRegionId);
...
function lifeOpportunities(
  resolution: LivingWorldResolution,
  receiptId: string
): readonly NpcLifeOpportunity[] {
  ...
  const values: NpcLifeOpportunity[] = [
    {
      ...
      targetId: `hub:${resolution.market.hubId}`,
      benefitBps: Math.max(2_000, 9_000 - riskBps),
      riskBps,
      distanceBps: 0,
    ...
    {
      id: `${receiptId}:resource`,
      kind: "resource",
      regionId: resolution.market.hubId,
      targetId: `commodity:${resolution.commodity}`,
      benefitBps: Math.max(2_000, Math.min(10_000, 10_000 - Math.min(8_000, stock * 10))),
      riskBps: Math.floor(riskBps / 3),
      distanceBps: 500,
      sourceReceiptId: receiptId,
      resolutionIndex: resolution.resolutionIndex,
    },
    {
      ...
      regionId: resolution.market.hubId,
      benefitBps: 6_000,
      riskBps: Math.floor(riskBps / 4),
      distanceBps: 250,
      sourceReceiptId: receiptId,
      resolutionIndex: resolution.resolutionIndex,
    },
    {
      id: `${receiptId}:reputation`,
      kind: "reputation",
      regionId: resolution.market.hubId,
      targetId: `polity:${resolution.market.hubId}`,
      benefitBps: 5_500 + Math.max(0, resolution.stabilityDelta) * 500,
      riskBps: Math.floor(riskBps / 2),
      distanceBps: 500,
      sourceReceiptId: receiptId,
      resolutionIndex: resolution.resolutionIndex,
    },
    {
      ...
      kind: "market",
      regionId: resolution.market.hubId,
      targetId: `market:${resolution.market.hubId}`,
      benefitBps: Math.min(10_000, 2_500 + resolution.unitPriceCopper * 12),
      riskBps: Math.floor(riskBps / 3),
      distanceBps: 300,
      sourceReceiptId: receiptId,
      resolutionIndex: resolution.resolutionIndex,
    ...
    {
      id: `${receiptId}:influence`,
      kind: "influence",
      regionId: resolution.caravan.destination ?? resolution.market.hubId,
      targetId: `polity:${resolution.caravan.destination ?? resolution.market.hubId}`,
      benefitBps: resolution.caravan.destination ? 8_000 : 4_500,
      riskBps,
      distanceBps: resolution.caravan.destination ? 2_500 : 800,
      sourceReceiptId: receiptId,
      resolutionIndex: resolution.resolutionIndex,
    },
  ];
...
function merchantRequestsFromResolution(
  worldSeed: string,
  resolution: LivingWorldResolution,
  ...
  const needEvents: NpcNeedEvent[] = [
    {
      id: `${receiptId}:wealth`,
      need: "wealth",
      delta: resolution.action === "trade" || resolution.action === "caravan" ? 0.08 : resolution.action === "consume" ? -0.04 : 0,
      sourceReceiptId: receiptId,
      resolutionIndex: resolution.resolutionIndex,
    },
    {
      id: `${receiptId}:safety`,
      need: "safety",
      delta: resolution.caravan.ambushed ? -0.18 : resolution.action === "patrol" ? 0.05 : 0,
      sourceReceiptId: receiptId,
      resolutionIndex: resolution.resolutionIndex,
    },
    {
      id: `${receiptId}:resources`,
      need: "resources",
      delta: resolution.action === "produce" ? 0.07 : resolution.action === "consume" ? -0.04 : 0,
      sourceReceiptId: receiptId,
      resolutionIndex: resolution.resolutionIndex,
    },
    {
      id: `${receiptId}:belonging`,
      need: "belonging",
      delta: resolution.action === "socialize" ? 0.08 : 0,
      sourceReceiptId: receiptId,
      resolutionIndex: resolution.resolutionIndex,
    },
    {
      id: `${receiptId}:status`,
      need: "status",
      delta: resolution.action === "patrol" ? 0.03 : resolution.action === "caravan" && !resolution.caravan.ambushed ? 0.02 : 0,
      sourceReceiptId: receiptId,
      resolutionIndex: resolution.resolutionIndex,
    },
    {
      id: `${receiptId}:power`,
      need: "power",
      delta: resolution.action === "caravan" && !resolution.caravan.ambushed ? 0.02 : 0,
      sourceReceiptId: receiptId,
      resolutionIndex: resolution.resolutionIndex,
    },
  ];
  ...
  const npcRequest: NpcRequest = {
    ...
    observationIds: Object.freeze([
      receiptId,
      `market:${resolution.market.hubId}:${resolution.commodity}:${resolution.unitPriceCopper}`,
    ]),
    memory: Object.freeze(newestMemory),
    opportunities: lifeOpportunities(resolution, receiptId),
    economy: {
      currentHubId: resolution.npc.currentHubId,
      wealthCopper: resolution.npc.wealthCopper,
      hungerBps: resolution.npc.hungerBps,
      fatigueBps: resolution.npc.fatigueBps,
      tradeProwessBps: resolution.npc.tradeProwessBps,
      harvestYieldBps: resolution.npc.harvestYieldBps,
    },
  };
  ...
  const worldRequest = Object.freeze({
    worldSeed,
    regionId: resolution.market.hubId,
    resolutionIndex: resolution.resolutionIndex,
    signals: [economySignal, politicsSignal],
  });

  const polityRequest = Object.freeze({
    polityId: `polity:${resolution.market.hubId}`,
    governmentType:
      resolution.market.hubId === "emberfall"
        ? ("trade_republic" as const)
        : resolution.market.hubId === "cinder_vault"
        ? ("warband" as const)
        : ("council" as const),
  ...
  return Object.freeze({
    resolution,
    npcRequest: Object.freeze(npcRequest),
    worldRequest,
    polityRequest,
    socialEvidence,
    receiptId,
...
export function prepareMerchantNpcDecision(input: {
  worldSeed: string;
  resolutionIndex: number;
  regionId: HubId;
  prior: NpcEconomySnapshot | null;
  social?: Readonly<{ action: LivingWorldSocialAction; sourceReceiptId: string }>;
}): MerchantDecisionRequests {
  const npc = confirmedNpcEconomy(input.regionId, input.prior);
  const market = merchantBootstrapMarkets[npc.currentHubId];
  const preferredGoal = input.prior?.decision?.goal;
  const resolution = resolveLivingWorldTick({
