...
import type { NpcNeedKey } from "./npcNeeds";
...
export function resolveWorldReaction(input: {
  ...
  const weatherTone: WorldReaction["weatherTone"] =
    totals.war > 0.6 || totals.hazard > 0.8
      ? "ashfall"
      : totals.weather > 0.55
      ? "storm"
      : totals.weather > 0.15
  ...
  const dialogueTone: WorldReaction["dialogueTone"] =
    threatDelta > 0.55 ? "urgent" : threatDelta > 0.2 || totals.politics > 0.3 ? "guarded" : "calm";
...
export function resolvePolityState(input: {
  ...
  const warPressure = clampUnit(
    input.warSignals
      .filter((signal) => signal.kind === "war" || signal.kind === "politics")
      .reduce((total, signal) => total + Math.max(0, signal.magnitude), 0) / 4
  );
  const stability = clampUnit(
    input.stability -
      warPressure * 0.2 +
      (activeDiplomacy.includes("alliance") ? 0.05 : 0) -
      (activeDiplomacy.includes("sanction") ? 0.05 : 0)
  );
  const reactionHash = canonicalHash([
    input.polityId,
