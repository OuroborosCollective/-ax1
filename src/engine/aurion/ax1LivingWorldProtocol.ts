import { sha256Hex, seed32FromHash } from "../math/DeterministicHash";
import type { NpcGoal } from "./npcNeeds";

export const AX1_LIVING_WORLD_RULESET = "aurion-ax1-living-world.v2" as const;

export const livingWorldSocialActions = [
  "negotiation",
  "diplomacy",
  "intimidation",
  "friendship",
...
export const commodityBasePrice: Readonly<Record<CommodityId, number>> = Object.freeze({
  grain: 20,
  sandstone: 45,
  bronze: 90,
  aether: 220,
...
export const productionFocus: Readonly<Record<HubId, readonly CommodityId[]>> = Object.freeze({
...
export const routeSecurity: Readonly<Record<string, number>> = Object.freeze({
...
export function marketPriceCopper(
  input: Readonly<{
    commodity: CommodityId;
    stock: number;
    demandBps: number;
...
export function socialMasteryEvidence(
  ...
  if (!sourceReceiptId || !Number.isSafeInteger(resolutionIndex) || resolutionIndex < 0) {
    throw new Error("social evidence requires server receipt and resolution index");
  }
  const mapping = {
...
export function resolveLivingWorldTick(
  input: Readonly<{
    worldSeed: string;
    resolutionIndex: number;
    market: MarketState;
  ...
  if (!input.worldSeed.trim() || !Number.isSafeInteger(input.resolutionIndex) || input.resolutionIndex < 0 || input.npc.currentHubId !== input.market.hubId) {
    throw new Error("invalid living world context");
  }
  let rng = seed32FromHash(`${input.worldSeed}:${input.resolutionIndex}:${input.npc.npcId}:${input.preferredGoal ?? "none"}:${AX1_LIVING_WORLD_RULESET}`);
  const roll = () => {
  ...
  const unitPriceCopper = marketPriceCopper({
    commodity,
  ...
  let action: LivingWorldResolution["action"] =
    hunger >= 7_500 ? "consume" : fatigue >= 8_500 ? "rest" : actionForGoal(input.preferredGoal, input.npc.wealthCopper, unitPriceCopper);

  const destinations = (["observatory_threshold", "windhollow", "emberfall", "cinder_vault"] as const).filter(
    (hub) => hub !== input.market.hubId
  );
  ...
  const taxCopper =
    action === "trade" || action === "caravan" ? Math.floor((unitPriceCopper * quantity * input.market.taxRateBasisPoints) / 10_000) : 0;
  const memoryEntry =
    action === "caravan"
      ? `trade:${destination}:${commodity}:${unitPriceCopper}:security=${securityIndex}${ambushed ? ":ambushed" : ""}`
      : action === "socialize"
      ? `social:${input.market.hubId}`
      : `${action}:${input.market.hubId}:${commodity}:${unitPriceCopper}`;
  ...
  const stockDelta =
    action === "produce"
      ? quantity
      : action === "consume" || action === "trade" || action === "caravan"
      ? -Math.min(quantity, input.market.stock[commodity])
      : 0;
  const market: MarketState = Object.freeze({
    ...input.market,
    treasuryCopper: input.market.treasuryCopper + taxCopper,
    stock: Object.freeze({
      ...input.market.stock,
      [commodity]: Math.max(0, input.market.stock[commodity] + stockDelta),
    }),
  });
  const wealthDelta =
    action === "consume"
      ? -Math.min(input.npc.wealthCopper, unitPriceCopper)
      : action === "trade" || action === "caravan"
      ? unitPriceCopper * quantity - taxCopper
      : 0;
  const hungerDelta =
    action === "consume"
      ? -3_000
  ...
  const npc: NpcEconomyState = Object.freeze({
    ...input.npc,
    currentHubId,
    wealthCopper: Math.max(0, input.npc.wealthCopper + wealthDelta),
    hungerBps: Math.round(clamp(hunger + hungerDelta, 0, 10_000)),
    fatigueBps: Math.round(clamp(fatigue + fatigueDelta, 0, 10_000)),
    memory: nextMemory,
  });
  ...
  return Object.freeze({
    ...
    market,
    npc,
    preferredGoal: input.preferredGoal ?? null,
    action,
    commodity,
    quantity,
    unitPriceCopper,
    taxCopper,
    caravan: Object.freeze({ destination, securityIndex, ambushed }),
    nextMemory,
    stabilityDelta,
    deterministicHash,
  });
