import { canonicalIntentIntake } from "./CanonicalIntentIntake";
import {
  canonicalizeClientIntent,
  chunkKeyFromWorldPosition,
  type ServerCanonicalIntent,
} from "./ServerCanonicalIntent";
import { sha256Hex, stableStringify } from "../math/DeterministicHash";
 ...
 *
 * The interaction gate enqueues a bounded transition intent. Player-bound session
 * state is applied on tick progression deterministically.
 */
export class AurionTransitionRuntime {
  ...
  applyReadyTransitions(currentTick: number): number {
    ...
    for (const entry of ready) {
      ...
      const targetZone: AurionZoneId =
        this.playerStates.get(entry.playerId)?.zoneId === AURION_EXPANSE_ZONE_ID
      ...
      this.playerStates.set(
        ...
        Object.freeze({
          ...
          zoneId: targetZone,
          entryPointId: targetZone === AURION_EXPANSE_ZONE_ID ? AURION_EXPANSE_ENTRY_POINT_ID : AURION_TOWER_RETURN_POINT_ID,
          returnPointId: targetZone === AURION_EXPANSE_ZONE_ID ? AURION_TOWER_RETURN_POINT_ID : AURION_EXPANSE_ENTRY_POINT_ID,
  ...
  private findLatestReceipt(playerId: string): AurionTransitionReceipt | null {
    ...
    return (
      [...receipts.values()].sort(
        (a, b) => b.sequenceId - a.sequenceId || binaryCompare(b.requestId, a.requestId)
      )[0] ?? null
    );
  }
}
