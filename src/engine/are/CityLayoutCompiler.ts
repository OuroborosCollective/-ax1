import { canonicalize } from "./WorldHashSnapshot";
...
function isBuilding(entity: CityLayoutEntity): boolean {
  return /house|building|hall|forge|wall|gate|road|archway|tower|portal/i.test(
    String(entity.type ?? entity.role ?? entity.id ?? "")
  );
}

function isRoad(entity: CityLayoutEntity): boolean {
  return /road|path|street|starpath/i.test(String(entity.type ?? entity.role ?? entity.id ?? ""));
}
...
export class CityLayoutCompiler {
  compileSector(entities: CityLayoutEntity[], sector: number): CityLayoutCompileResult {
    const scoped = entities
      .filter((entity) => sectorOf(entity) === sector && isBuilding(entity))
      .map((entity) => canonicalize(entity) as CityLayoutEntity);
