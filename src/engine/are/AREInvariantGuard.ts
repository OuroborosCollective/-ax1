/**
 * AREInvariantGuard.ts
 *
 * Enforces determinism axioms and invariant checks across the ARE simulation kernel.
 * Validates tick sequences, kappa invariants (1000 kappa per tile/standard precision),
 * and scans sources for non-deterministic forbidden tokens (Date.now, Math.random, etc.).
 */
...
export class AREInvariantGuard {
  ...
  scanAndRecord(source: string, file?: string, tickTimestamp = 0): AREInvariantGuardStatus {
    const violations = this.scanCoreSource(source, file);
    if (violations.length > 0) {
      const scanResult: SourceScanResult = { file: file ?? "unknown", violations, scannedAt: tickTimestamp };
