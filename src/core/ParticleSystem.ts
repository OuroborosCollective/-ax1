...
export class ParticleSystem {
  ...
  private maxParticles: number = 2400;
  ...
  private emitCombatHit(pos: THREE.Vector3, color: THREE.Color, count: number) {
    for (let i = 0; i < count; i++) {
      ...
      this.particles.push({
        ...
        color: color.clone().offsetHSL((Math.random() - 0.5) * 0.1, 0, (Math.random() - 0.5) * 0.2),
        size: 0.35 + Math.random() * 0.35,
        initialSize: 0.35 + Math.random() * 0.35,
        alpha: 1.0,
        initialAlpha: 1.0,
        life: 0,
  ...
  private emitCombatCrit(pos: THREE.Vector3, color: THREE.Color, count: number) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const ringSpeed = 6.0 + Math.random() * 4.0;

      this.particles.push({
        position: pos.clone().add(new THREE.Vector3(0, 0.2, 0)),
        velocity: new THREE.Vector3(Math.cos(angle) * ringSpeed, 1.2 + Math.random() * 2.0, Math.sin(angle) * ringSpeed),
        acceleration: new THREE.Vector3(0, -6.0, 0),
        color: new THREE.Color(0xfbbf24).lerp(color, Math.random() * 0.4),
        size: 0.55 + Math.random() * 0.4,
        initialSize: 0.55 + Math.random() * 0.4,
        alpha: 1.0,
        initialAlpha: 1.0,
        life: 0,
  ...
  private emitSlashArc(pos: THREE.Vector3, color: THREE.Color, count: number) {
    for (let i = 0; i < count; i++) {
      ...
      const pPos = pos.clone().add(new THREE.Vector3(Math.sin(arcAngle) * radius, 0.3 + (Math.random() - 0.5) * 0.4, Math.cos(arcAngle) * radius));

      this.particles.push({
        position: pPos,
        velocity: new THREE.Vector3(Math.sin(arcAngle) * 3.0, (Math.random() - 0.2) * 2.0, Math.cos(arcAngle) * 3.0),
        acceleration: new THREE.Vector3(0, -3.0, 0),
        color: color.clone(),
        size: 0.4 + Math.random() * 0.3,
        initialSize: 0.4 + Math.random() * 0.3,
        alpha: 0.9,
        initialAlpha: 0.9,
        life: 0,
  ...
  private emitMagicImpact(pos: THREE.Vector3, color: THREE.Color, count: number) {
    for (let i = 0; i < count; i++) {
      ...
      this.particles.push({
        ...
        initialSize: 0.45 + Math.random() * 0.35,
        alpha: 1.0,
        initialAlpha: 1.0,
        life: 0,
  ...
  private emitLevelUpFlare(pos: THREE.Vector3, count: number) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this.particles.push({
        ...
        initialSize: 0.6 + Math.random() * 0.5,
        alpha: 1.0,
        initialAlpha: 1.0,
        life: 0,
  ...
  private emitBeaconBurst(pos: THREE.Vector3, color: THREE.Color, count: number) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const pPos = pos.clone().add(new THREE.Vector3(Math.cos(angle) * radius, Math.random() * 1.5, Math.sin(angle) * radius));

      this.particles.push({
        position: pPos,
        velocity: new THREE.Vector3(-Math.sin(angle) * 3.0, 7.0 + Math.random() * 4.0, Math.cos(angle) * 3.0),
        acceleration: new THREE.Vector3(0, 2.0, 0),
        color: color.clone().offsetHSL((Math.random() - 0.5) * 0.1, 0, 0.1),
        size: 0.5 + Math.random() * 0.4,
        initialSize: 0.5 + Math.random() * 0.4,
        alpha: 1.0,
        initialAlpha: 1.0,
        life: 0,
  ...
  private emitSteamPuff(pos: THREE.Vector3, count: number) {
    for (let i = 0; i < count; i++) {
      if (this.particles.length >= this.maxParticles) break;
      this.particles.push({
        position: pos.clone().add(new THREE.Vector3((Math.random() - 0.5) * 0.8, Math.random() * 0.4, (Math.random() - 0.5) * 0.8)),
        velocity: new THREE.Vector3((Math.random() - 0.5) * 1.2, 2.5 + Math.random() * 2.5, (Math.random() - 0.5) * 1.2),
        acceleration: new THREE.Vector3((Math.random() - 0.5) * 0.5, 0.8, (Math.random() - 0.5) * 0.5),
  ...
  private emitSmokePuff(pos: THREE.Vector3, count: number) {
    for (let i = 0; i < count; i++) {
      if (this.particles.length >= this.maxParticles) break;
      this.particles.push({
        position: pos.clone().add(new THREE.Vector3((Math.random() - 0.5) * 0.6, Math.random() * 0.3, (Math.random() - 0.5) * 0.6)),
        velocity: new THREE.Vector3((Math.random() - 0.5) * 0.8, 1.8 + Math.random() * 2.0, (Math.random() - 0.5) * 0.8),
        acceleration: new THREE.Vector3(0, 0.5, 0),
        color: new THREE.Color(0x64748b),
        size: 0.5 + Math.random() * 0.5,
        initialSize: 0.5 + Math.random() * 0.5,
        alpha: 0.55,
        initialAlpha: 0.55,
        life: 0,
  ...
  private emitExplosion(pos: THREE.Vector3, color: THREE.Color, count: number) {
    for (let i = 0; i < count; i++) {
      ...
      const pColor = isFire ? new THREE.Color(0xf97316).lerp(new THREE.Color(0xfacc15), Math.random()) : color.clone();
  ...
  private emitHealSparkles(pos: THREE.Vector3, count: number) {
    for (let i = 0; i < count; i++) {
      ...
      const pPos = pos.clone().add(new THREE.Vector3(Math.cos(angle) * radius, 0.2 + Math.random() * 0.5, Math.sin(angle) * radius));

      this.particles.push({
        position: pPos,
        velocity: new THREE.Vector3((Math.random() - 0.5) * 0.5, 2.5 + Math.random() * 2.0, (Math.random() - 0.5) * 0.5),
        acceleration: new THREE.Vector3(0, 0.5, 0),
        color: new THREE.Color(0x10b981).lerp(new THREE.Color(0x34d399), Math.random()),
        size: 0.45 + Math.random() * 0.35,
        initialSize: 0.45 + Math.random() * 0.35,
        alpha: 1.0,
        initialAlpha: 1.0,
  ...
  private emitBloodOilSplash(pos: THREE.Vector3, count: number) {
    for (let i = 0; i < count; i++) {
      ...
      this.particles.push({
        position: pos.clone().add(new THREE.Vector3((Math.random() - 0.5) * 0.3, 0.8, (Math.random() - 0.5) * 0.3)),
        velocity: new THREE.Vector3(Math.cos(theta) * speed, 1.0 + Math.random() * 3.0, Math.sin(theta) * speed),
        acceleration: new THREE.Vector3(0, -12.0, 0),
        color: Math.random() > 0.5 ? new THREE.Color(0x991b1b) : new THREE.Color(0x334155),
        size: 0.3 + Math.random() * 0.25,
        initialSize: 0.3 + Math.random() * 0.25,
        alpha: 0.9,
        initialAlpha: 0.9,
        life: 0,
  ...
  private emitTeleportWarp(pos: THREE.Vector3, color: THREE.Color, count: number) {
    for (let i = 0; i < count; i++) {
      ...
      const pPos = pos.clone().add(new THREE.Vector3(Math.cos(angle) * radius, Math.random() * 2.5, Math.sin(angle) * radius));

      this.particles.push({
        position: pPos,
        velocity: new THREE.Vector3(-Math.cos(angle) * 3.0, 3.0 + Math.random() * 4.0, -Math.sin(angle) * 3.0),
  ...
  public update(delta: number) {
    ...
    for (const beacon of this.activeBeacons) {
      if (beacon.active) {
        beacon.timer += delta;
        if (Math.random() < 0.4) {
          const angle = Math.random() * Math.PI * 2;
          ...
          if (this.particles.length < this.maxParticles) {
            this.particles.push({
              position: pPos,
              velocity: new THREE.Vector3(-Math.sin(angle) * 1.5, 4.5 + Math.random() * 2.5, Math.cos(angle) * 1.5),
              acceleration: new THREE.Vector3(0, 1.0, 0),
              color: beacon.color.clone(),
              size: 0.45 + Math.random() * 0.3,
              initialSize: 0.45 + Math.random() * 0.3,
              alpha: 0.85,
              initialAlpha: 0.85,
              life: 0,
    ...
    for (let i = 0; i < this.particles.length; i++) {
      ...
      this.colors[aliveCount * 3] = p.color.r * p.alpha;
      this.colors[aliveCount * 3 + 1] = p.color.g * p.alpha;
      this.colors[aliveCount * 3 + 2] = p.color.b * p.alpha;
